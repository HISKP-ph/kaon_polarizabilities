...
/// Turn off default GSL error handling, sucht that error handling with exceptions can be invoked.
struct TurnOffGslErrorsHelper {
    TurnOffGslErrorsHelper() {gsl_set_error_handler_off();}
};

/// Turn off the default GSL error handling as soon as this header file is included.
class TurnOffGslErrors { 
    const static TurnOffGslErrorsHelper t;
};

void check(int status);
    ///<@brief Check whether a `status` returned by a GSL routine indicates failure.
...
struct Integration {
    virtual Value operator()(const Function& f, double lower, double upper) const=0;
        ///<@brief Integrate the function `f` in the interval [`lower`,`upper`].
