...
class OnOffStream {
public:
    OnOffStream() : status{true}, stream{std::cerr} {}
    OnOffStream(bool b, std::ostream& out) : status{b}, stream{out} {}

    void on() {status=true;}
    void off() {status=false;}

    template<class T>
    OnOffStream operator<<(const T& a)
    {
        if (status)
            stream<<a;
        return *this;
...
/// Template for linspace comparable to the python routine
template<typename T>
